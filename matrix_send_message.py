...
def log(send_email=False,text=u"Произошёл сбой в скрипте %s" % sys.argv[0]):
	...
	if send_email:
		...
		for email_to in conf.email_admin:
			mail.sendmail(text=text, subj=u"ОШИБКА при обработке команды!", send_to=email_to, server=conf.email_server, files=[conf.log_path] , send_from=u"matrix@zabbix.rs.int")
